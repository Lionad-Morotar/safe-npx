...
export function parseArgs(argv: string[]): ParsedArgs {
  ...
  let pkgSpec: string | null = null;
  let pkgName: string | null = null;
  const npxPrefixArgs: string[] = []; // npx flags that go BEFORE package (e.g., -y)
  const restArgs: string[] = []; // args that go AFTER package
 ...
 */
export function buildOptions(snpxFlags: Partial<SnpxFlags>): CliOptions {
  ...
  const parsedHours = parseFloat(timeHours as string);
